...
fn test_compare_directory_sizes() {
    ...
    // Create files with different content (different size) - 1 byte difference
    fs::write(dir1_path.join("diff_size.txt"), "123").unwrap();
    fs::write(dir2_path.join("diff_size.txt"), "1234").unwrap();

    fs::create_dir_all(dir1_path.join("nested")).unwrap();
    fs::create_dir_all(dir2_path.join("nested")).unwrap();
    
    // Create files with different sizes in nested directory (same name, different sizes) - 44 byte difference
    fs::write(dir1_path.join("nested").join("nested_file.txt"), "only1_nested").unwrap();
    fs::write(dir2_path.join("nested").join("nested_file.txt"), "only2_nestedonly2_nestedonly2_nestedonly2_nestedonly2_nestedonly2_nestedonly2_nestedonly2_nested").unwrap();
    ...
    // Verify output with -s flag contains ~ for files with different sizes (shows old -> new sizes)
